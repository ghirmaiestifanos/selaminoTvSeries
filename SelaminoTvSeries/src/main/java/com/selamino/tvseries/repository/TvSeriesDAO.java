...
public class TvSeriesDAO {
	...
	public Map<String, Object> getEpisodesList(int tvSeriesId){
		...
		String sql = "select s.seasonNumber, ep.title, ep.profile_image, group_concat(tg.gener SEPARATOR ', ')AS gener, ep.episodeNumber, ep.cover_image, ep.videoLink, "
				+ "ep.description, ep.episodeId "
				+ "FROM tvseries tv "
				...
				+ "where tv.tvseriesId = ? group by ep.title, s.seasonNumber, ep.profile_image, ep.episodeNumber, ep.cover_image, ep.videoLink, ep.description, ep.description, ep.episodeId "
